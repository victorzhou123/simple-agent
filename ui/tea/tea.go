...
func New(cfg Config, onSubmit SubmitFunc) ui.UI {
	ta := textarea.New()
	...
	ui := &uiImpl{
		send:      p.Send,
		program:   &Program{p: p},
		formatter: newFormatter(cfg.Formatter),
	}
...
func (u *uiImpl) Run() error { return u.program.Run() }
...
func (u *uiImpl) ShowToolCall(toolName string, input map[string]any) {
	u.send(chunkMsg(u.formatter.FormatToolCall(toolName, input)))
}

func (u *uiImpl) ShowToolResult(toolName string, output string, err error) {
	u.send(chunkMsg(u.formatter.FormatToolResult(toolName, output, err)))
}
